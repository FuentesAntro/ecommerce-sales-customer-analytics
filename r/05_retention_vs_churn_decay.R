# 05_retention_vs_churn_decay.R - FIXED
library(ggplot2)
library(scales)
...
p <- ggplot(df, aes(x = Month)) +
  ...
  # FIX: usar annotate para que no herede las 9 filas
  ...
  annotate("curve", x = 1.8, y = 0.25, xend = 1.12, yend = 0.195, color = "#2E86AB", curvature = 0.2, arrow = arrow(length = unit(0.02, "npc"))) +
  annotate("curve", x = 1.8, y = 0.71, xend = 1.12, yend = 0.795, color = "#E74C3C", curvature = -0.2, arrow = arrow(length = unit(0.02, "npc"))) +
  ...
  labs(title = "Retention vs Churn Decay — Average Cohort", subtitle = "Avg M1 Retention 19% | 25 cohorts | 24-month window", x = NULL, y = NULL, color = NULL) +
...
# Guarda solo 1 vez - cambia la ruta si tu proyecto no esta en Desktop/howells
ggsave("retention_vs_churn_19pro.png", plot = p, width = 10, height = 5.5, dpi = 300)
